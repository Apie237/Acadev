...
export default function CourseConnectSidebar({ 
  ...
  return (
    <div
      className={`${
        open ? "w-64" : "w-16"
      } bg-white border-r border-[#BAD0CC]/30 h-screen transition-all duration-300 flex flex-col shadow-lg fixed top-0 left-0 z-40`}
      style={{ marginLeft: 0 }}
    >
      {/* Header */}
      ...
      <div className="p-3">
        <div
          ...
          className={`flex items-center ${open ? 'gap-3 px-3' : 'justify-center'} py-2 rounded-lg cursor-pointer transition-all ${
            selectedCourse === null
        ...
        >
          <Globe size={18} />
          {open && <span className="font-medium text-sm">General</span>}
        </div>
      </div>
        ...
        <div className="space-y-1">
          {enrolledCourses.map((course) => (
            <div
              ...
              className={`flex items-center ${open ? 'gap-3 px-3' : 'justify-center'} py-2 rounded-lg cursor-pointer transition-all group ${
            ...
            >
              ...
              {open && <span className="font-medium text-sm line-clamp-1">{course.title}</span>}
            </div>
          ))}
        </div>

        {enrolledCourses.length === 0 && open && (
